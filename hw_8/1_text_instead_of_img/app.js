...
let allButter = document.querySelectorAll('button');
// 1.1 затем проитерируйтесь по кнопкам и каждой из
// них добавьте обработчик клика - функцию handleClick
allButter.forEach(e => e.addEventListener('click', handleClick));
...
function hideMoreText(card) {
    ...
    card.productName.querySelector('.desc').remove();
...
function showMoreText(card) {
    ...
    card.productName.insertAdjacentHTML('beforeend', `<div class="desc">${newText}</div>`);
